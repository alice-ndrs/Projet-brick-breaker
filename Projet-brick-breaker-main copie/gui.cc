...
enum Response { CANCEL, OPEN_FILE, SAVE_FILE };

enum Buttons { EXIT, OPEN, SAVE, RESTART, START, STEP };

constexpr unsigned drawing_size(500);

My_window::My_window(string file_name)
    : m_game(), last_file(file_name), main_box(Gtk::Orientation::HORIZONTAL),
...
void My_window::exit_clicked() { hide(); }
...
bool My_window::key_pressed(guint keyval, guint keycode, Gdk::ModifierType state) {
    if (!level_loaded && (keyval == 's' || keyval == '1')) { return true; }
...
void My_window::dialog_response(int response, Gtk::FileChooserDialog *dialog) {
    ...
    if (dialog->get_file()) {
        ...
        if (file_name.extension() != ".txt") { file_name = ""; }
    }
    ...
    case OPEN_FILE:
        if (file_name != "") { open_file(file_name); }
        break;

    case SAVE_FILE:
        if (file_name != "") { save_file(file_name); }
        break;
...
void My_window::draw_bricks(const Cairo::RefPtr<Cairo::Context> &cr) const {
    for (const auto &brick : m_game.get_bricks()) {
        ...
        case BrickType::RAINBOW: {
            static const Color rainbow_colors[] = {RED,  ORANGE, YELLOW, GREEN,
                                                   CYAN, BLUE,   PURPLE};
            int hp = brick->get_hit_points();
            if (hp >= 1 && hp <= 7) {
                set_color(rainbow_colors[hp - 1]);
            } else {
                set_color(WHITE);
            }
...
void My_window::on_drawing_move(double x, double y) {
    ...
    if (init_x > arena_size) { init_x = arena_size; }
}